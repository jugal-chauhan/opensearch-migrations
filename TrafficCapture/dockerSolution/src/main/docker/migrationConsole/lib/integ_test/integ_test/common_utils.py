...
def clear_snapshots(cluster: Cluster, repository: str) -> None:
    ...
    try:
        ...
        response = execute_api_call(cluster=cluster, path=snapshots_path, expected_status_code=200)
        ...
        for snapshot in snapshots:
            ...
            execute_api_call(cluster=cluster, path=delete_path, method=HttpMethod.DELETE, expected_status_code=200)
            logger.info(f"Deleted snapshot: {snapshot_name} from repository '{repository}'.")
    except Exception as e:
        logger.warning(f"Error clearing snapshots from repository '{repository}': {e}")
...
def delete_repo(cluster: Cluster, repository: str) -> None:
    ...
    try:
        ...
        logger.warning(f"Error deleting repository '{repository}': {e}")
